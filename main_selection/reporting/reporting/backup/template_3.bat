...
::Details of package
set packageName=Package2
set package_path=c:\rvd\rvd_portal\cypress\specs\prototyping\*.*
...
:: ------ Search for latest folder created (if it's not found the batch will EXIT) if it's found, "echo" will write the message in CMD window
FOR /F %%j IN ('dir %backup_location% /a:d /t:c /o-d /b') DO (
    SET b=%%j
    GOTO :found_last
)
EXIT
...
xcopy /s /i /y /a %reports_location%\%last_subfolder%\%packageName% %backup_location%\%b%\%date_folder%\%packageName%
